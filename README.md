...
| **Builder** | Implements one approved design inside scope, quotes changed lines in `implementation_summary.md`, runs `review-check-implementation`, then exits. The orchestrator owns the submit-test loop, retry budget, and failure-log reinjection. |
